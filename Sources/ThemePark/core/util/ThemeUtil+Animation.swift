...
extension ThemeUtil {
   ...
   public static func transition(_ view: UIView) {
      UIView.animate(withDuration: Theme.transitionDur, animations: { apply(view) }, completion: nil)
   }
   /**
    * For Controllers that are not a View it self
    */
   public static func transition(_ viewController: UIViewController) {
      UIView.animate(withDuration: Theme.transitionDur, animations: { apply(viewController) }, completion: nil)
   }
}
#endif
